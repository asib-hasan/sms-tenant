                            ...
                            <div class="card card-primary card-outline card-outline-tabs">
                                <div class="card-header p-0 border-bottom-0">
                                    <ul class="nav nav-tabs" id="custom-tabs-four-tab" role="tablist">
                                        <li class="nav-item">
                                            <a class="nav-link" href="{{ route('atmgt/report/student') }}">By Student</a>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link active" href="{{ route('atmgt/report/employee') }}">By Employee</a>
                                        </li>
                                    </ul>
                                </div>
                                <div class="card-body">
                                    <div class="tab-pane fade show active">
                                        <div class="container">
                                            <form action="{{ route('atmgt/report/employee') }}" method="get">
                                                @csrf
                                                <div class="row mt-0 pt-0">
                                                    <div class="form-group col-md-6">
                                                        <label for="Name">Employee ID</label>
                                                        <input type="text" name="employee_id" class="form-control" value="{{ $employee_id }}">
                                                    </div>
                                                    <div class="form-group col-md-6">
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body table-responsive p-2 table-bordered">
                                        <table class="table table-hover text-nowrap" id="dataTable">
                                            <tbody>
                                            <tr>
                                                <th style="background-color: #ddd" colspan="2">Employee Information</th>
                                            </tr>
                                            <tr>
                                                <td>Name <br>
                                                    <b>{{ $employee_info->first_name }} {{ $employee_info->last_name }}</b>
                                                </td>
                                                <td>ID <br> <b>{{ $employee_info->employee_id }}</b></td>
                                            </tr>
                                            <tr>
                                                <td>Designation <br>
                                                    <b>{{ $employee_info->designation_info->name ?? '--' }}</b>
                                                </td>
                                                <td>Phone <br>
                                                    <b>{{ $employee_info->phone }}</b>
                                                </td>
                                            </tr>
                                            </tbody>
                                        </table>
...
{{--                                                    $attendance_info = $attendance_records->where('employee_id',$employee_id)->where('date',$startDate->format('Y-m-d'))->first();--}}
                                        ...
                                        <table class="mt-3 table table-hover text-nowrap" id="dataTable">
                                            ...
                                            @while ($startDate->lte($endDate))
                                                @php
                                                    $week_day = $startDate->dayOfWeek;
                                                    $off_day = $weekDays[$week_day] ?? null;
                                                    $attendance_info = $attendance_records->where('user_id',$employee_id)->where('date',$startDate->format('Y-m-d'))->first();
                                                    $holiday_info = \App\Models\HolidayModel::where('date',$startDate->format('Y-m-d'))->first();
                                                @endphp
                                                @if($off_day && $off_day->is_active == 1)
                                                    @php $startDate->addDay();@endphp
