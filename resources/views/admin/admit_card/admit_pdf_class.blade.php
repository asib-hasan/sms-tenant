...
<div class="admit-card">
    <table style="width: 100%;">
        ...
        <tr>
            <td style="font-size: 15px;font-weight: bold;padding-top: 20px;width: 20%">ID No: {{ $std->student_id }}</td>
            <td colspan="2" class="admit-card-btn" style="padding-left: 70px">
                <button style="font-weight: bold">Admit Card</button>
            </td>
            <td style="text-align: right;">
                @if($std->student_info && $std->student_info->photo)
                <img style="height: 80px;width: 80px; border:1px solid black;padding:1px;" src="{{ asset('uploads/students/' . $std->student_info->photo) }}" alt="">
                @else
        ...
        <tr>
            ...
            <td style="width: 40%;border-bottom: 1px solid black">{{ $std->student_info->first_name ?? '' }} {{ $std->student_info->last_name ?? '' }}</td>
            <td style="width: 10%;white-space: nowrap">Father's Name:</td>
            <td style="width: 40%;border-bottom: 1px solid black">{{ $std->student_info->father ?? '' }}</td>
        </tr>
        <tr>
            <td style="width: 10%">Class:</td>
            <td style="width: 40%;border-bottom: 1px solid black">{{ $std->class_info->name ?? '' }}</td>
            <td style="width: 10%; white-space: nowrap">Mother's Name:</td>
            <td style="width: 40%;border-bottom: 1px solid black">{{ $std->student_info->mother ?? '' }}</td>
        </tr>
        <tr>
            <td style="width: 10%;">Session:</td>
            <td style="width: 40%;border-bottom: 1px solid black">{{ $std->session_info->name ?? '' }}</td>
